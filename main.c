...
int main() {
    ...
    // Show main menu and loop until quiz is selected or quit
    int choice;
    do {
        choice = afficher_ecran_accueil(ecran);
        printf("Main: Received choice=%d\n", choice);
        if (choice == -1) { // Quit on window close
            break;
        }
    } while (choice != 1); // Loop until quiz is selected

    if (choice != 1) { // Exit if not quiz (e.g., SDL_QUIT)
        printf("Exiting game (choice=%d)\n", choice);
    ...
    while (running && e.vies > 0 && e.level <= 10) {
        ...
        while (!answered && e.vies > 0 && running) {
            ...
            // Display the current question (includes timer animation)
